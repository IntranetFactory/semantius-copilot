...
## 5.5: one rule per spec surface; pure-skeleton template; §6 handoff sub-sections

`CURRENT_VERSION` `5.4` → `5.5` (2026-08-19). **Minor**: every change is additive or a clarification of a rule the spec already implied; no canonical section is renumbered, no existing table column is removed or reshaped, no frontmatter key is removed. The modeler's `EXPECTED_MAJOR = 5` is unchanged and reads 5.4 and 5.5 files alike (its Stage 1 accepts both forms wherever one changed).

**What changed and why.** A pipeline run spent a large share of its reasoning reconciling contradictions between the template, SKILL.md's pre-save table, the stage references, the modeler's parse rules, and the optimizer's emitter. Root cause: the template's ```markdown fence mixed the emitted skeleton with ~150 lines of authoring commentary (so the analyst and the optimizer each guessed what to emit), conditional keys added over time got explicit emit-rules while older surfaces kept unconditional prose, and several spec-side docs still used blueprint section numbers. Nothing mechanical caught it (see the architect CHANGELOG for `consistency-check.ts`).

**The new convention.**

1. **Template = "Placeholder conventions" + a pure "Skeleton" fence + per-section "Authoring guidance".** The skeleton (`## Skeleton starts below this line` … `## Skeleton ends above this line`) is emitted byte-for-byte and contains zero em-dashes; all explanatory text was relocated verbatim into guidance. `{{name?}}` marks an optional line/key (omit the whole line when unset; never `null`); `{{mermaid_block}}` is the generated §2 diagram. An offline lint (`semantius-optimizer/evals/round-trip/check.ts`) asserts zero em-dashes in the skeleton and that every skeleton heading / emitted table header appears in the optimizer.
2. **H1** is `# <system_name>: Semantic Model` (colon). The modeler also accepts the pre-5.5 `—` form.
3. **Empty / not-applicable cells are `(none)`**: §8.1 `reconciliation` (when not `re-prefixed-from …`), §9.1 RACI realization `consult_mode`, `raci_assignments` `consult_mode` (`read | notify | block | (none)`). The `**Processes:**` caption is bare; its empty form is the inline `**Processes:** _(none: …)_`.
4. **`persona`** is emitted only under `access_scope: full` when §9 carries a RACI matrix; absent under `basic`. SKILL.md's pre-save row that required it unconditionally (and `module_kind: null`) is corrected.
5. **`module_kind`** is optional: carried verbatim when the blueprint has it, otherwise the key is omitted (never `null`).
6. **`**Label column:**`** is REQUIRED on every non-junction entity and OPTIONAL on `entity_type: junction` (omit the line when the junction has no distinct local label; never invent a synthetic label field). `**Catalog owner:**` (already parsed by the modeler and emitted by the optimizer) now appears in the skeleton after `**Entity type:**`.
7. **§6** is the link table (now with the `Reconciliation` column the analyst already wrote per stage-11 delta 3) followed by two unnumbered sub-sections `### Outbound handoffs` / `### Inbound handoffs` carrying the blueprint's §6.2 / §6.3 rows verbatim (nine columns, both access scopes, `_(none: …)_` when empty). Every spec-side "§6.2 / §6.3" reference (stage-11 delta 6, SKILL.md gate row, stage-9-governance Step 5, stage-3-collisions, stage-2-inspect, stage-1-parse) is retargeted. `to_state` validation applies to Outbound `lifecycle` rows (the `payload` entity's `workflow_state` enum); Inbound payloads belong to the source module.
8. **§9.1 reconciliation cells** carry exactly `✨ to create` / `♻ exists` / `🟡 drift on module_id` (bare; `♻` U+267B); the stage references' descriptive marks are narration. §8.1 gains the `narrow` example row; `tier` is the modeler's accepted set.
9. **Blueprint-numbered references on spec surfaces fixed**: SKILL.md "§5 rows carry delete_mode/fk_format" → §4 (from the blueprint's §5); "permission row in §2" → §8.1; "no §7 lifecycle state is gated" → no `workflow-gate` row; modes :39 and stage-3f-drift "§7 `requires_permission?`" → `require_permission` rule / §9.1 `process_gates`; "An approval is … a §7 gated transition" → a gated transition.
10. **stage-11-write.md** no longer carries a second frontmatter block; the template's is canonical and Stage 11 lists only its carry-forward/omit rules. A single `---` follows the last §3 entity block.

Files: `references/semantic-spec-template.md` (restructured), SKILL.md, `references/stage-11-write.md`, `stage-9-governance.md`, `stage-1-parse.md`, `stage-2-inspect.md`, `stage-3-collisions.md`, `stage-4-fields.md`, `stage-3f-drift.md`, `modes-audit-extend-rebuild.md`.
...
Guidance only, no contract change, `CURRENT_VERSION` unchanged (2026-08-18). The analyst now uses the harness task tools (`TaskCreate` / `TaskUpdate` / `TaskList` / `TaskGet`; canonical rules in `../semantius-admin/references/task-tracking.md`; ordering via `blocks` / `blockedBy`: stage tasks chained, pointed at the admin pipeline task when orchestrated, and every `Q:` task blocking its stage task): a resident "Task tracking" paragraph in SKILL.md; the Stage pipeline index gains a Task subject column (four `Match ›` stage tasks in reconcile mode, one per non-reconcile mode); ledger stages are access scope, 3a-3e, 3f, and Extend's Stage 2/3 re-run: every widget becomes a `Q:` task (subject = the exact question text; a multiSelect over four choices is several "(i of N)" tasks) before the first widget fires, questions are asked in batches of up to four per `AskUserQuestion` call, and a task is completed only after its answer is in `customizations.yaml`; the 3g plan render and closing confirmation require the ledger clean; the MUST-FIRE rule is restated mechanically (a MUST-FIRE widget is a `Q:` task that must reach `completed`). The 3a "successive calls until all are covered" wording is replaced by the ledger; the sentence claiming batched questions inherit the first question's header is removed (verified false: each question carries its own header chip), so optional-parts questions may share a call. The 3g confirmation and its follow-ups stay standalone; "revise" re-opens the affected `Q:` tasks. Files: SKILL.md, `stage-3-collisions.md`, `stage-3f-drift.md`, `stage-3-confirm.md`, `access-control-scope.md`, `customizations-consultation.md`, `modes-audit-extend-rebuild.md`.
