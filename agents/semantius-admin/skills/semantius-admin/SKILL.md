...
- **INV-8 — Bash never carries file content, and heredocs are banned in tool-emitted bash outright.** No `cat > file <<EOF` to write a file, no `semantius call … <<'JSON'` to feed the CLI, no multi-KB `printf`/`echo` bodies. Rationale: a model-emitted generation can be cut mid-stream, and a heredoc cut before its terminator leaves the command blocked on stdin **indefinitely** (a live session wedged ~55 minutes this way), while a truncated pipe form fails immediately as a visible bash syntax error. File content goes through the Write tool, chunked per the parts protocol ([`references/parts-protocol.md`](./references/parts-protocol.md)) when it can exceed ~4 KB. Small inline payloads (≤ ~1 KB, ASCII, no apostrophes/backticks) use the fail-fast pipe form `printf '%s' '…' | semantius call …`; anything larger or with hazardous quoting goes through a file (`semantius call … < payload.json`) or a Bun script. PowerShell here-strings (`@'…'@`) fall under the same ban. Tiny fixed writes stay legal (`printf 'version: "1.0"\n' > file` seeds, `yq -i` scalar writes, one-line log appends) — fail-fast, no open stdin. A truncated generation is an expected, recoverable event: rewrite the affected part or payload file; never retry as a heredoc. Each sub-skill carries this rule residently (they also run standalone).
