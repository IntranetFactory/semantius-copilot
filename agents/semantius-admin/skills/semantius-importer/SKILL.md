...
6. **`AskUserQuestion` fires alone.** Apply mapping edits and re-renders first, in earlier steps, then call `AskUserQuestion` as the only tool call of its response; a sibling tool call in the same response cancels the pause and the run continues before the user has answered. The answers arrive as a `<user_answers>` input block; there is no `user_answers` tool, never call one.
...
**One pre-write confirmation gate** (`AskUserQuestion`, never a typed y/n): render the complete numbered plan — module ops, entity, each field, planned alterations, then the import (rows and batches) — and get one approval before the first write. Run the helpers in an earlier step; the gate's `AskUserQuestion` is the only tool call of its response. Every number and field line in the plan comes from the helpers, not from prose arithmetic: `bun run render-plan.ts` for the mapping table and counts, `bun run create-fields.ts --dry-run` for the exact `create_field` payloads. The per-topic decisions above happen during analysis; the gate is a single yes.
