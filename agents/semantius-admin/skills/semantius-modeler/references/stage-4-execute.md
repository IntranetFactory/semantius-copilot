...
```
Module → Permissions → ALL Entities (one create_entity call) → Fields (one create_field call per entity, only after every entity exists) → rules / spine → …
```

**Two rules govern every write in this stage.** (1) **Batch: more than one record of the same kind pending → ONE call.** Every typed `create_*` takes `data` as an array (items may have different keys), every `update_*` / `delete_*` takes an `id` array (same `data` for all); the response is always an array. All entities of the spec go out in one `create_entity` call, all of an entity's fields in one `create_field` call, the baseline permissions / hierarchy / roles / grants as one call per row kind. A loop of single-record `create_*` calls is a defect. (2) **Every entity before any field.** 4c completes for the whole spec before 4d issues its first `create_field`, so a field whose `reference_table` names an entity declared later in §3, a self-reference, or a promote-create row resolves immediately — there is no second pass. A bulk call is one transaction: a failed call landed **nothing**; fix the cause and re-run (never fall back to per-row calls). Read-before-write stays: one `read_*` with an `in.(...)` filter over the whole set (`readIn` / `ensureMany` in `deploy-lib.ts`), never a `--single` read per record.

<!-- DUPLICATE of canonical copy in SKILL.md (Cross-cutting safety invariants). Edit both. -->

**Failure is loud and halting (the recovery model depends on it).** The deploy's entire recovery story is re-run convergence: the spec is the target, every Stage 4 op is read-before-write and idempotent, and a failed or partial deploy is recovered by **re-running** — there is no transaction, rollback, or resume (PostgREST is stateless). That model is only safe if a partial failure is **visible**. So when any Stage 4 sub-stage's write fails (a `create_*` / `update_*` / `postgrestRequest` returns non-zero, a platform constraint trips, or a ⚠ row forces a FAIL LOUD), **stop immediately and tell the user the deploy is incomplete and must be re-run** — do not swallow the error, do not continue to the next sub-stage, and never let the closing message or the Stage 5 summary print a success-shaped result over a partial write. The single way this model breaks in practice is a partial failure that reads as success, so the operator never re-runs. State the halt plainly (and within the Writing Conventions, no em-dashes in this user-facing line): *"Deploy halted at `<sub-stage>` after N writes. The deploy is incomplete: fix the cause and re-run, and the modeler reconciles forward from wherever it stopped (every op is idempotent, so re-running never double-creates)."* This is especially load-bearing inside 4k living-mode, which materializes the RACI engine across five separate `postgrestRequest` batches (`processes` → `raci_assignments` → `process_gates` → enforcement rules → `raci_mode` flag): a mid-sequence abort there must surface, never be summarized away. On a halt the `Apply › Update your live model` task stays `in_progress` with `halted: <the message above>` appended to its description (SKILL.md → Task tracking); it is never marked completed over a partial write.

**When this stage needs a script (prose-bearing payloads, many writes, or conditional logic over the live catalog — the common case), build it from [`references/deploy-script-template.md`](./deploy-script-template.md), which assembles the deploy around the committed primitives in [`references/deploy-lib.ts`](./deploy-lib.ts).** Those primitives are the concrete implementation of the loud-and-halting invariant above: an exit-code-aware `read1` / `readIn` for read-before-write (never a `try/catch` existence probe), the loud `write` for mutations, the bulk `ensureMany` / `ensurePairs` / `createMany` (one array call per set of records, ids from a re-read), the create-or-diff field sync (not create-if-missing), and a `runDeploy` harness that halts non-zero on the first failure and never prints success over a partial deploy. Copy `deploy-lib.ts` into the scratch dir and import it; do **not** invent your own error handling — the `intgov-inventory` deploy reported success at ~40% complete because hand-rolled `createEntityIfMissing` / `addFieldIfMissing` helpers wrapped writes in a bare `catch` and continued. **Even batched, a large model is dozens of CLI calls (one `create_entity`, one `create_field` per entity, the read sweeps and drift updates) and can run past a default tool timeout, so run `bun run` in the background (or with a raised tool timeout), not as a default-timeout foreground call that gets killed mid-deploy. If it is killed partway, re-run the same script — read-before-write skips whatever already landed (a bulk call is all-or-nothing, so no half batch exists) and it converges; never hand-finish a partial deploy by creating the remaining entities manually.** The template's "Large deploys" note has the full rule.
...
**Scaffold pass.** The committed [`references/scaffold-lib.ts`](./scaffold-lib.ts) `scaffoldModule()` implements this entire pass as one idempotent call (module + permissions + hierarchy + roles + the six FK wires + provenance, with a live-schema preflight on field names); a deploy script should call it rather than re-implement steps 1-5 by hand (hand-rolling is what produced orphan roles and the `user:read` module header). The contract it executes: after 4a's module create-or-update, run the standard scaffold (Stage 2a-scaffold steps 2–5): create the permissions per tier **as one set** (one `permission_name=in.(…)` read, ONE `create_permission` call carrying every missing tier, one re-read for the ids, one id-array `update_permission` to converge a drifted `module_id`), create the hierarchy chain **as one set** tagged `origin = "model"` (domain) or `"model_master"` (master), create the default roles per tier **as one set** using the §9.1-resolved role slugs (read verbatim, not reconstructed from the module slug) with `origin` tagged matching the module type **and `catalog_role_code` stamped from the §9.1 baseline-role slug** (lineage; VALUE-only, write-once), attach the `role_permissions` **as one set**, and populate the six module-record FK / column references. Each step is idempotent on re-run (a re-run issues no creates). Surface the three-permission upgrade case as `✨ <slug>:admin / <slug>_admin role` plan lines.
...
Read the existing edges first — one `read_permission_hierarchy` with `including_permission_id=in.(<all including ids>)`, matched by `(including, included)` pair locally (`ensurePairs` in `deploy-lib.ts` does exactly this; re-runs are idempotent) — then create every missing edge in **one** `create_permission_hierarchy` call. **Never invert direction** — the narrower permission must never appear on the including side (that would mean the narrower one "includes" the broader, which breaks RBAC).

**Re-run reconciliation.** When the module already exists on the two-permission baseline but the current model has been upgraded to need three (any §3 entity now carries `**Edit permission:** admin`), the deploy adds the missing `<slug>:admin` permission and the missing `admin → manage` hierarchy row additively. Surface this in the Stage 3 plan as `✨ saas_expense_tracker:admin` and `✨ admin → manage` so the user can see the upgrade. Never delete or rename existing permissions or hierarchy rows.
...
**4c. Entities**, Walk model §2 in order and apply each entity's bucket decision. **This pass runs to completion for the whole spec before 4d creates a single field.** The buckets that provision an entity (✨ New, 🛑 rename incoming, 📥 promote-create) do not each issue a `create_entity`: collect their payloads (built per the checklist below) and issue **ONE `create_entity` call** for all of them after one `read_entity` sweep with `table_name=in.(<every table>)` (`ensureMany("read_entity", "table_name", …, "create_entity", entityRows)` in `deploy-lib.ts` — it re-reads afterwards, so the entity ids come from the read, never from the create response). Because every entity of the deploy exists before any field is created, a `reference_table` that names an entity declared later in §3, a self-reference, or a promote-create row in the master module resolves immediately in 4d. The buckets that update an existing host (♻️, 🛑 merge, promote-existing) stay per entity, since each patch differs. Any 🛑 rename-existing `update_entity` runs **before** the create call (rename-both: rename first, then the incoming joins the batch).
...
Complete payload — copy this shape; fill every value from §3, do not reconstruct from memory. One such object per provisioned entity, **all of them in one array** (`ensureMany(..., "create_entity", entityRows)` adds the `{data: [...]}` envelope; shown here as the raw call for the shape):

```typescript
await call("create_entity", {
  data: [{                                     // ← ONE call, one object per entity of the spec
...
- **Field absent live** → goes into the entity's single `create_field` array call as before (auto-generated fields `id`, `label`, `created_at`, `updated_at`, the entity's `label_column`, and the composed-label columns `_label` / `<fk>_label` are still skipped).
- **Field present live** → compute the property delta against the model and emit **one** `update_field` carrying every changed key. Issue one call per drifted field (not one per property) so the audit log records a coherent change set per column; when several fields need the **same** patch (e.g. `unique_value: true` on every label column carrying the `unique` marker, or the `"disabled"` migration below across many computed columns), send that patch once with an `id` array. Properties to compare:
...
Each entry carries an optional `source_module` field. The deployer sets it automatically when emitting an entity update: the value is the `system_slug` of the model currently being deployed. Entries without `source_module` (admin-edited via the UI, or created outside the deploy pipeline) are treated as `source_module = "user"` for rule purposes.
...
**4g. Built-in extensions.** If the user confirmed additive field extensions on a built-in (e.g. the model declares `users.department_id` and the built-in doesn't have it), create those fields after all custom entities are done — all of them in **one** `create_field` call (`ensureMany("read_field", "id", (f) => \`${f.table_name}.${f.field_name}\`, "create_field", rows)`, keyed by the composite id since they span tables). Do not modify existing built-in fields, do not change formats or enum values.

**No second pass.** Because 4c creates every entity of the spec before 4d creates any field, self-reference fields (e.g. `departments.parent_department_id` → `departments`) and cross-reference pairs (e.g. `departments.manager_user_id` ↔ `users.department_id`, where `users` is a built-in and the `users` side is a 4g extension) are created in the normal 4d / 4g calls — nothing has to wait. Only 4g (built-in extensions) and 4h (§6 links) still run after the in-module field pass, and each is one `create_field` array call.

**Spine pass (`label_parent`).** After 4d (and 4g) — so every `reference`/`parent` FK now exists, self-references included — stamp each freshly-created entity's identity spine. For every ✨-new or rename-incoming entity whose spec carries a `**Label parent:**` line, `update_entity` with `data.label_parent = "<fk_field>"`. **Read-before-write:** skip the call when the live entity's `label_parent` already equals the spec value (idempotent re-run). This is deferred out of 4c for the same reason 4e defers `computed_fields` / `validation_rules` — the value names a field that must exist first, and the platform validates it at write time; stamping it on `create_entity` rejects whenever the FK has not been created yet. The ♻️ same-module branch in 4c already re-points `label_parent` via `update_entity` on drift (its fields exist), so it needs no spine-pass entry; only newly-created entities do. Stage 5 still round-trips `label_parent` against the spec, so a missed stamp surfaces in verification.

After the field pass, share one UI link per entity:
...
**4h. Cross-model link suggestions.** After all in-module creates and built-in extensions are done, walk the Proposed list from Stage 3 and execute the confirmed rows as additive fields — **all of them in one `create_field` call** (they span source tables, so key by the composite id: `ensureMany("read_field", "id", (f) => \`${f.table_name}.${f.field_name}\`, "create_field", rows)`). **Read-before-write:** that helper reads the composite ids first; a target FK column (`<target_singular>_id`, or the user-supplied alternative) that already exists landed on a prior deploy — it is left out of the create as a clean idempotent no-op, not re-created and not counted as a failure.
...
# One call for every confirmed row — `data` is an array (shown with one item; append the others).
...
**Skip silently** for any Stage-3 confirmed proposal the platform rejects (e.g. the resolved target was renamed between Stage 2g inspection and 4h write). Because the confirmed rows go out as one all-or-nothing call, a rejected row fails the whole call and none of the §6 links land: drop the rejected row from the set (the platform error names it), re-issue the one call for the rest, and surface the rejected row in the verification summary; do not retry the rejected row. (An *already-exists* result is not a rejection — the read-before-write check above turns it into a clean no-op, so re-runs don't pad the "parked" count with rows that actually landed on a prior deploy.) Skipped, ambiguous-and-skipped, dormant, and resolved-but-declined rows are listed in the verification summary so the user can see how many §6 hints landed and how many parked.
...
```bash
# Read original manager role members, and the master role's current members (one read each)
semantius call crud read_user_role '{"filters": "role_id=eq.<original_manager_role_id>"}'
semantius call crud read_user_role '{"filters": "role_id=eq.<master_manager_role_id>"}'
# ONE create_user_role call carrying every member not yet in <master>_manager (idempotent by the diff above;
# `ensurePairs("read_user_role", "role_id", "user_id", "create_user_role", rows)` in deploy-lib.ts does exactly this)
semantius call crud create_user_role '{"data": [{"user_id": <user_id_1>, "role_id": <master_manager_role_id>}, {"user_id": <user_id_2>, "role_id": <master_manager_role_id>}]}'
```
...
2. **Grant resolution per RACI row.** For each parsed §9.1 RACI row whose `actor` is this persona AND `kind = persona`:
   ...
   - Idempotent grants, as one set per persona: one `read_role_permission` with `role_id=eq.<persona_role_id>` (every existing grant of the role), diff locally, then **one** `create_role_permission` call carrying every missing `{role_id, permission_id}` pair (`ensurePairs("read_role_permission", "role_id", "permission_id", "create_role_permission", rows)`). The grant FK points at whatever module currently owns the gate's entity.
   - Also grant the persona on the entity's write tier (`<entity_owning_module>:<tier>` where `<tier>` is the suffix of the entity's resolved `edit_permission` — i.e. the `manage` / `admin` / `<narrow_suffix>` value Stage 1 derived from the entity's §3 `**Edit permission:**` line). The spec carries the write tier as that `**Edit permission:**` line, not as a literal `write tier` column (that column name is the blueprint's; the analyst transforms it into `**Edit permission:**` on emission). Same set: it is one more pair in the persona's single `create_role_permission` call.
   - For `raci = consulted`: grant an advisory read on the gate's owning module (`<entity_owning_module>:read`). Same set, same one call. The row's `consult_mode` (`read` / `notify` / `block`) is **not** behaviorally distinguished in `documentation` mode — all three collapse to the advisory-read grant. The `notify` / `block` semantics are realized only in `living` mode (carried on the `raci_assignments` row and enforced by the `has_consultation` C-block rule + the `emits_events` notify trigger; see the living-mode materialization below).
...
**Living mode — materialize the RACI matrix + enforcement.** When the resolved RACI mode is `living`, after role + write-tier grants, materialize the analyst's RACI plan via the generic `postgrestRequest` tool (the `crud` server exposes no dedicated `create_process` verb — use `{method, path, body}` against the registered RACI tables). All emissions are idempotent and batched per table: one GET over the natural keys (`in.(...)`), then **one POST per table with an array `body`** carrying every missing row (raw PostgREST: identical keys on every row of the array — send `null` / `""` explicitly, never omit a key; `postMany` in `deploy-lib.ts` asserts this), i.e. GET-the-set → POST-the-missing rather than GET/POST per row:
...
3. Idempotent grants as one set: read the roles' existing `role_permissions` (`role_id=in.(...)`), diff locally, **one** `create_role_permission` call for every missing pair (`ensurePairs`).
...
2. **Mint catalog-prefixed sibling permissions.** For each `<old_prefix>:<verb>` resolve `<new_prefix>:<verb>`; the whole set goes through one `ensureMany("read_permission", "permission_name", …, "create_permission", rows)` — one `in.()` read, **one** `create_permission` call for the missing siblings (`module_id = <new owner's module id>`, `description` copied from the source permission row), one re-read for the ids.

3. **Sibling role_permissions.** For each `role_permissions` row whose `permission_id` is one of the old-prefixed permissions, the sibling pair `(role_id, <new_perm_id>)` — all pairs in **one** `create_role_permission` call after one `role_id=in.(...)` read (`ensurePairs`). **No deletion** (per the no-auto-deletion symmetric rule); the old-prefixed rows remain as quiet orphans.

4. **Sibling permission_hierarchy.** For each `permission_hierarchy` row referencing an old-prefixed permission (in either `including_permission_id` or `included_permission_id`): re-emit the equivalent edge against the new-prefixed permission — all edges in **one** `create_permission_hierarchy` call after one read over the including ids (`ensurePairs`). Old rows remain (no-delete).
