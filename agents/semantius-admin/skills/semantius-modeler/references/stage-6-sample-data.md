...
**Generate a single Bun (TypeScript) script** for all sample data rather than making individual CLI calls. This avoids context bloat from dozens of sequential tool invocations. Write the script under `<cwd>/.tmp_deploy/seed_<short>.ts`, run it once with `bun run`, check the output, and delete it.

**Seed scripts over ~4 KB are written via the parts protocol** ([`../../semantius-admin/references/parts-protocol.md`](../../semantius-admin/references/parts-protocol.md)) — with several entities' pools and loop blocks they routinely exceed the chunk cap, and a generation cut mid-`Write` ships a script that silently seeds only the first tables. Parts under `.tmp_deploy/parts/seed-<short>/`: part `01` carries the imports and the `COUNT` / `perEntity` / `eligibleTables` config, one part per entity's pools + loop block, and the **final part ends with the `assertSeedCounts(...)` call** (so the R5 tail check doubles as proof the count guard survived assembly). Manifest first, sentinel per part, verification pass, assemble to `candidate.ts`, syntax-gate it (`bun build "parts/seed-<short>/candidate.ts" --target=bun --outdir "parts/seed-<short>/build"` — a parse failure means a truncated part; discard the build output), then `mv` to `.tmp_deploy/seed_<short>.ts`. A truncated part is rewritten alone, never the whole script, and never via a bash heredoc. **Never write generated scripts into the skill folder or the working directory root.** They are ephemeral one-shots; persisting them across runs accumulates as catalog drift, mixes throw-away artifacts with skill source, and survives session boundaries. See the "Generated artifacts" section above for the full rule.
