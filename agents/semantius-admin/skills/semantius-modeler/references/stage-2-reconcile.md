...
3. **Create the permission-hierarchy chain — as one set.** Resolve both ends of every row in the spec's §9.1 Permission hierarchy table from `permission_name` to id once at the top, then **read-before-write over the set** — one `read_permission_hierarchy` filtered by `including_permission_id=in.(<every including id>)`, match the `(including, included)` pairs locally, and issue **one `create_permission_hierarchy` call** carrying every missing edge (`including_permission_id = <row.permission>.id`, `included_permission_id = <row.includes>.id`, `origin = "model"` (domain) or `"model_master"` (master)). A re-run finds every chain row already present and issues no call (same guard 4b restates). `ensurePairs` in `deploy-lib.ts` is this step as code.
