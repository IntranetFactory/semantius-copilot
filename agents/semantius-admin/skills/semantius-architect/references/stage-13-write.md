...
**Write sequence — the blueprint is emitted in verified parts, never one giant generation.** A blueprint can run tens of KB, and a model-emitted tool call can be cut mid-stream at any point; requiring the file to survive one `Write` is how a truncation ships a partial artifact or none at all. The chunked-write mechanics (chunk cap ~4 KB, manifest, sentinel, verification pass, assembly pipeline, recovery) are the shared **parts protocol** — read [`../../semantius-admin/references/parts-protocol.md`](../../semantius-admin/references/parts-protocol.md) before the first `Write`. The sequence:

1. **Create the parts folder fresh** (empty) per protocol R2: `.tmp_admin/<run_id>/parts/<system_slug>/` when admin-orchestrated; on a standalone run mint a `run-<timestamp>` folder first. `Write` the `00-manifest` (protocol R3) listing every planned `NN-<label>.md` part.
2. **`Write` the blueprint as parts** (protocol R1/R2/R4): one section per part, ~4 KB max each, each ending with the `<!-- part-complete -->` sentinel line; split any over-cap section across consecutive parts. Unlike the analyst's spec, the §2 Mermaid diagram is **legitimately hand-built** here (per `stage-5-mermaid.md`'s build-then-verify procedure) — the diagram section is simply one of the parts, no placeholder fence needed.
3. **Run the verification pass** (protocol R4). A missing, truncated, or unterminated part → rewrite **only that part** and re-verify (protocol R7); never regenerate the whole blueprint, never fall back to a bash heredoc.
4. **Assemble to the candidate** (protocol R5) with the canonical pipeline into `parts/<system_slug>/candidate.md`; verify it is non-empty and its tail matches the last part's closing content.
5. **Run the resident Pre-save verification and `consistency-check.ts` against the candidate** (SKILL.md; the mechanical gate is what catches a hand-built diagram that drifted from §3/§5).
6. **Move the candidate into place**: `mv parts/<system_slug>/candidate.md semantius/blueprints/<system_slug>-semantic-blueprint.md`. The committed path never holds a half-finished state. After this the parts are dead — never reassemble; any later revision is an `Edit` on the final file (capped per R1), and revise-loop edits re-run the pre-save gates.
